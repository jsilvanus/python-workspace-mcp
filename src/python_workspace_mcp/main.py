...
def get_system_info() -> dict:
    ...
    return {"server_version": __version__, "api_version": "1", "mcp_sdk_major": 2, "deployment_profile": "local", "transport": "streamable-http", "user": users.info(_current_user_id()), "runtime": {"execution_backend": "docker", "docker_image": settings.docker_image}, "workspace": {"count": len(visible), "default_workspace_id": settings.default_workspace_id}, "capabilities": {"persistent_workspace": True, "multiple_workspaces": len(visible) > 1, "docker_execution": True, "artifacts": True, "resource_limits": True, "resource_profiles": True, "on_demand_resources": True, "network_access": False, "non_root_runtime": True, "read_only_runtime_filesystem": True, "multiple_users": True, "file_catalog": True, "execution_history": True}, "limits": default.limits.as_dict(), "maximum_limits": default.maximum_limits.as_dict(), "resource_profile": default.profile_id}
